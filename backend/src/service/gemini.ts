...
export async function transcribeAudio(audioAsBase64: string, mimeType: string) {
  const response = await gemini.models.generateContent({
    ...
    contents: [
      {
        text: 'Transcreva o áudio para português do Brasil. Seja preciso e natura na transcrição. Mantenha a pontuação e divida o texto em parágrafos quando for apropriado.',
      },
      {
        inlineData: {
          mimeType,
